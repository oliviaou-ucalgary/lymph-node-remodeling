# Optional CCL21A gene-expression plots

# Import libraries and data ------------------------------------------------------
source(file.path("config", "analysis_config.R"))
source(file.path("R", "reproducibility.R"))
require_packages(c("Seurat", "ggplot2", "patchwork"))
suppressPackageStartupMessages({ library(Seurat); library(ggplot2); library(patchwork) })
if (!exists("ccl21a_joint") || !inherits(ccl21a_joint, "Seurat")) stop("Run scripts/05_ccl21a_joint_clustering.R first.", call. = FALSE)
validate_seurat(ccl21a_joint, assay = "RNA", metadata = c("group", "seurat_clusters"))

# Genes previously used included: Cxcl13, Ccl21a, and Ccl19.
genes_to_plot <- c(
  # Add mouse gene symbols here.
)

# Actual analysis ---------------------------------------------------------------
if (!length(genes_to_plot)) {
  message("No optional genes specified; edit genes_to_plot to display plots.")
} else {
  missing_genes <- setdiff(genes_to_plot, rownames(ccl21a_joint))
  if (length(missing_genes)) stop("Genes not found: ", paste(missing_genes, collapse = ", "), call. = FALSE)
  ccl21a_feature_plot <- FeaturePlot(ccl21a_joint, features = genes_to_plot, split.by = "group")
  ccl21a_violin_plot <- VlnPlot(ccl21a_joint, features = genes_to_plot, group.by = "seurat_clusters", split.by = "group", pt.size = 0)
  print(ccl21a_feature_plot)
  print(ccl21a_violin_plot)
}

